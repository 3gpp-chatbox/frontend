...
export const JsonToMermaid = (jsonData, options = {}) => {
  ...
  const {
    direction = "TD",
    styles = {
      entity: { fill: "#f9f", stroke: "#333", "stroke-width": "2px", color: "#000" },
      action: { fill: "#bbf", stroke: "#333", "stroke-width": "2px", color: "#000" }
    }
  } = options;
  ...
  graphData.nodes.forEach(node => {
    ...
    const closeShape = nodeType === "action" ? "))" : "]";

    // Build node content
    let nodeContent = `**${node.type.toUpperCase()}**<br>`;
    
    if (node.label) {
      nodeContent += `${node.label}<br>`;
    }
